...
.PARAMETER Repeats
    Number of monitoring cycles (1 = single run, 2+ = multiple runs with inter-cycle waits)

.PARAMETER InterCycleWait
    Wait time BETWEEN monitoring cycles (in seconds)
...
function Show-Help {
    ...
    Write-Host "  -WaitTime <seconds>       Wait before starting monitoring (initial stabilization)"
    Write-Host "  -RunTime <seconds>        Monitoring duration (0=until completion)"
    Write-Host "  -Repeats <count>          Number of monitoring cycles (default=1, single run)"
    Write-Host "  -InterCycleWait <seconds> Wait between monitoring cycles (for multi-run)`n"
    ...
    Write-Host "  # Run with multiple monitoring cycles (3 cycles with 10s wait between)"
    Write-Host "  .\RunTest.ps1 -TestID GLD-1001 -SoCWatch -Repeats 3 -InterCycleWait 10`n"
...
# Build config file path based on script type (perf or power)
$basePath = $PSScriptRoot

# Load Power KPI test list from TestConfig.ps1
$testConfigPath = Join-Path $PSScriptRoot "TestConfig.ps1"
if (Test-Path $testConfigPath) {
    $powerKPIs = . $testConfigPath
} else {
    Write-Host "Warning: TestConfig.ps1 not found. Using default classification." -ForegroundColor Yellow
    $powerKPIs = @('GLD-1001', 'GLD-1002', 'GLD-1003', 'GLD-1004', 'GLD-1005', 'GLD-1006', 'GLD-1007', 'GLD-6002')
}

# Determine test type based on TestID
if ($powerKPIs -contains $TestID) {
    $testType = 'POWER_KPI'
    Write-Host "Test Type: Power KPI" -ForegroundColor Magenta
} else {
    $testType = 'PERF_KPI'
    Write-Host "Test Type: Performance KPI" -ForegroundColor Cyan
}

$configFileName = "$TestID.config.ps1"
$configFilePath = Join-Path (Join-Path $basePath $testType) $configFileName

# Check if config file exists
if (-not (Test-Path $configFilePath)) {
    Write-Host "Error: Config file not found: $configFilePath" -ForegroundColor Red
    Write-Host "Searched in: $testType folder" -ForegroundColor Yellow
    Write-Host "`nAvailable config files in POWER_KPI:" -ForegroundColor Yellow
    $powerConfigs = Get-ChildItem -Path (Join-Path $basePath "POWER_KPI") -Filter "*.config.ps1" -ErrorAction SilentlyContinue
    if ($powerConfigs) {
        $powerConfigs | ForEach-Object {
            Write-Host "  - $($_.BaseName -replace '\.config$', '')" -ForegroundColor Cyan
        }
    } else {
        Write-Host "  (None found)" -ForegroundColor Gray
    }
    
    Write-Host "`nAvailable config files in PERF_KPI:" -ForegroundColor Yellow
    $perfConfigs = Get-ChildItem -Path (Join-Path $basePath "PERF_KPI") -Filter "*.config.ps1" -ErrorAction SilentlyContinue
    if ($perfConfigs) {
        $perfConfigs | ForEach-Object {
            Write-Host "  - $($_.BaseName -replace '\.config$', '')" -ForegroundColor Cyan
        }
    } else {
        Write-Host "  (None found)" -ForegroundColor Gray
...
# Override timing parameters if specified
if ($Repeats -ge 1) { $config['Repeats'] = $Repeats }
if ($InterCycleWait -ge 0) { $config['InterCycleWait'] = $InterCycleWait }
if ($WaitTime -ge 0) { $config['WaitTime'] = $WaitTime }
...
if ($WaitTime -ge 0 -or $RunTime -ge 0 -or $Repeats -ge 1 -or $InterCycleWait -ge 0) {
    Write-Host "Timing Parameters: " -NoNewline -ForegroundColor Cyan
    $timingParams = @()
    if ($WaitTime -ge 0) { $timingParams += "WaitTime=$($WaitTime)s" }
    if ($RunTime -ge 0) { $timingParams += "RunTime=$($RunTime)s" }
    if ($Repeats -ge 1) { $timingParams += "Repeats=$Repeats" }
    if ($InterCycleWait -ge 0) { $timingParams += "InterCycleWait=$($InterCycleWait)s" }
    Write-Host ($timingParams -join ", ") -ForegroundColor Yellow
}
